...
class ACEncoder(VAE.EncoderBase):
    def __init__(self, nfilters, re_term,
                 enc_dim=520,
                 enc_dim_desc={'hidden_num': 512, 'class_num': 8},
                 enc_shape=[None, 49, 70, 1], name_scope='EncoderSkeleton'):
...
class AEDecoder(VAE.DecoderBase):
    def __init__(self, nfilters, re_term):
        super(AEDecoder, self).__init__([None, 128], [None, 8], [None, 1, 70, 1], 'DecoderSkeleton')
        self.nfilters = nfilters
        self.re_term = re_term
...
class Discriminator(VAE.DiscriminatorBase):
    def __init__(self, nfilters, re_term):
        super(Discriminator, self).__init__([None, 75, 70, 1],
                                            8,
                                            "DiscriminatorSkeleton")
        self.nfilters = nfilters
        self.encoder = ACEncoder(nfilters, re_term,
                                 enc_shape=[None, 75, 70, 1],
                                 enc_dim=512,
                                 enc_dim_desc={'hidden_num': 512},
